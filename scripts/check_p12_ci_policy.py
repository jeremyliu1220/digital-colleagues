...
"""Validate the exact read-only, branch-independent P12 current CI policy."""
...
jobs:
  current-ci:
    name: Current P12 governance non-publishing verification
...
def check_ci_policy(root: Path) -> dict[str, object]:
    ...
    if any(token.lower() in lowered for token in forbidden):
        raise P12GateError("P12 CI contains a forbidden trigger, authority, or command")
    if workflow.count("run: make ci") != 1:
        raise P12GateError("P12 workflow current gate invocation is not exact")
    ...
    if (
        ...
        raise P12GateError("P12 Node/npm repository identity is inconsistent")
    return {
        "schema_version": 1,
        "gate": "p12_ci_policy",
        "status": "passed",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"P12 CI policy check failed: {exc}", file=sys.stderr)
        return 2
