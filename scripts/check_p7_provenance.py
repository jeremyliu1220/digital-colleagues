...
"""Validate complete P7 change provenance with zero parent-source migration."""
...
from scripts.check_p7_repository import BASE_COMMIT

RECEIPT = "provenance/p7-migration-receipt.json"
EXCLUDED = {RECEIPT, "artifacts/p7/summary.json"}
FIELDS = {"destination", "classification", "implementation_basis", "gate_result"}
BASIS = "public_documents_and_accepted_p6_implementation"


class ProvenanceError(RuntimeError):
    """P7 provenance coverage is incomplete or unsafe."""
...
def _git(root: Path, *arguments: str) -> tuple[str, ...]:
    ...
    if completed.returncode != 0:
        raise ProvenanceError("Git P7 change inventory failed")
...
def framed_digest(root: Path, paths: set[str]) -> str:
    ...
    for relative in sorted(paths):
        ...
        if not document.is_file():
            raise ProvenanceError("a P7 implementation destination is missing")
...
def check_provenance(root: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise ProvenanceError("P7 receipt is unreadable") from exc
    if not isinstance(receipt, dict) or set(receipt) != {
        ...
        raise ProvenanceError("P7 receipt shape is unsupported")
    if receipt["schema_version"] != 1 or receipt["milestone"] != "P7":
        raise ProvenanceError("P7 receipt identity is unsupported")
    if receipt["source_basis"] != BASIS or receipt["transformed_entries"] != []:
        raise ProvenanceError("P7 source classification drifted")
    entries = receipt["new_implementations"]
    if not isinstance(entries, list):
        raise ProvenanceError("P7 implementation entries must be a list")
    destinations: set[str] = set()
    for entry in entries:
        if not isinstance(entry, dict) or set(entry) != FIELDS:
            raise ProvenanceError("a P7 implementation entry has unsafe fields")
        if (
            entry["classification"] != "new_implementation"
            or entry["implementation_basis"] != BASIS
            or entry["gate_result"] != "not_a_source_migration"
        ):
            raise ProvenanceError("a P7 implementation classification is invalid")
        destination = _safe_path(entry["destination"])
        if destination in destinations:
            raise ProvenanceError("P7 receipt destinations are duplicated")
        destinations.add(destination)
    actual = changed_files(root)
    if destinations != actual:
        raise ProvenanceError("P7 receipt does not cover the complete change inventory")
    return {
        "schema_version": 1,
        "gate": "p7_provenance_clean",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate complete P7 provenance.")
    parser.add_argument("root", nargs="?", default=".")
    arguments = parser.parse_args(argv)
    try:
        result = check_provenance(Path(arguments.root).resolve())
    except (OSError, ProvenanceError) as exc:
        print(f"P7 provenance check failed: {exc}", file=sys.stderr)
        return 2
